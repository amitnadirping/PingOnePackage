...
/// Public class that contains data of authetnticate with code response
SWIFT_CLASS("_TtC10PingOneSDK20AuthenticationObject")
@interface AuthenticationObject : NSObject
/// List of users that are paired to this device
@property (nonatomic, copy) NSArray<NSDictionary<NSString *, id> *> * _Nullable users;
/// State whether user approval is required or not. Possible values are: REQUIRED, NOT_REQUIRED
@property (nonatomic, copy) NSString * _Nullable userApproval;
/// Client context object retirved from server
@property (nonatomic, copy) NSString * _Nullable clientContext;
/// The status of the authentication: CLAIMED, EXPIRED, DENIED or COMPLETED
...
/// \param completionHandler Will return NSError in case of an error.
///
- (void)denyWithUserId:(NSString * _Nonnull)userId completionHandler:(void (^ _Nonnull)(NSString * _Nullable, NSError * _Nullable))completionHandler;
...
/// Claim autheticate code with the SDK.
///
/// returns:
/// a completionHandler that contains users list, clientContext object,
/// userApproval String and status String. In case userApproval value is REQUIRED,
/// the AuthenticationObject will be used in order to Approve or Deny the Authentication Code.
